...
const ExpensesPage = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`http://${window.location.hostname}:5000/api/expenses`, formData, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleDelete = async (id, e) => {
    ...
    if (!passcode) return;
    try {
      await axios.delete(`http://${window.location.hostname}:5000/api/expenses/${id}`, {
        headers: { 
  ...
  const handleEditSave = async (id, e) => {
    ...
    if (!passcode) return;
    try {
      await axios.put(`http://${window.location.hostname}:5000/api/expenses/${id}`, editData, {
        headers: { 
          Authorization: `Bearer ${token}`,
          'X-Passcode': passcode 
        }
