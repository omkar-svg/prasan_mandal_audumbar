...
const MembersPage = () => {
  ...
  const fetchMembers = async () => {
    try {
      const res = await axios.get(`http://${window.location.hostname}:5000/api/members`, {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`http://${window.location.hostname}:5000/api/members`, formData, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleDelete = async (id, e) => {
    ...
    if (!passcode) return;
    try {
      await axios.delete(`http://${window.location.hostname}:5000/api/members/${id}`, {
        headers: { 
